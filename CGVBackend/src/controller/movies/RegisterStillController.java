...
public class RegisterStillController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		MultipartRequest mr = FileUtils.upload(req, req.getServletContext().getInitParameter("STILLS_DIRECTORY"));
		if(mr != null){ //업로드 성공 시		
			...
			if(affected==1){
				...
				//업로드 된 파일 지우기
				FileUtils.deleteFile(req, req.getServletContext().getInitParameter("STILLS_DIRECTORY"), mr.getFilesystemName("poster"));
				//입력 값 그대로 두기
