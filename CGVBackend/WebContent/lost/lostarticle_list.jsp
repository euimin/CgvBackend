...
<title>CGV 관리자 페이지-분실물 관리</title>
<!-- Bootstrap core CSS -->
<link rel="stylesheet"
...
var checkDelete = function(){
	...
	if(checkedNo.length != 0){
		...
		if(confirm("선택한 항목을 삭제 하시겠습니까?")){
			location.href="<c:url value='/lost_delete.cgv?"+checkString+"'/>";
		}
	}
};
			...
			<div class="table-responsive">
				<div align="center">
					<div align="right">
						<div>
							<select>
								<option value="[아이디로검색]">아이디</option>
					...
					<tbody>
						<c:forEach var="list" items="${list}" varStatus="loop">
							<tr>
								...
								<td>${list.place}</td>
								<td>${list.title}</td>
								<td><a href="<c:url value='/lost.view.cgv?no=${list.no}'/>">${list.content}</a></td>
